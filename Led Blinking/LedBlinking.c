...
void main() {
  ...
  do {
    ...
    LATA = 0xA;       // Turn ON LEDs on PORTA
    LATB = 0xA;       // Turn ON LEDs on PORTB
    LATC = 0xA;       // Turn ON LEDs on PORTC
    LATD = 0xA;       // Turn ON LEDs on PORTD
    LATE = 0xA;       // Turn ON LEDs on PORTE
    Delay_ms(1000);    // 1 second delay
